...
**배운점**:
- test-case의 예상 결과 - 실제 결과의 비교를 통해 P/F 처리 결정
- 예상 결과는 모호하게 하지 말고 직관적으로 작성
- 정보 표시 TC의 테스트 방법 중 자동/수동을 고르기 힘들 때 2단계로 분리하여 작성하는 방법이 있음.
- 정보가 있는지? > 자동
- 해당 정보가 올바른지? > 수동
- 테스팅은 시크릿 모드에서 진행
- config.py : 설정 값 저장소
...
- ul[class*='RecentSearchList] 처럼 동적 클래스일경우 공통된 단어를 *를 통해 다음과 같이 찾기 가능
